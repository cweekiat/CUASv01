...
class YOLOv8(nn.Module):
    def __init__(self, ResBlock, n_classes, n_blocks_list=[3, 4, 6, 3],
        ...
        n = [0.33, 0.25, 2.0] # hyperparameters for scaling the number of channels

        self.conv = nn.Sequential(nn.Conv2d(in_channels=num_channels,
                                            out_channels=int(64*n[0]), kernel_size=3,
                                            stride=2, padding=1),
                                  nn.BatchNorm2d(int(64*n[0])),
                                  nn.SiLU())
        # First layer
